...
int k_fs_read_write(descriptor_t *desc, void *buffer, size_t size, int op)
{
	...
	if (op) {
		...
		size_t indeks_bloka = block_start;
		size_t indeks_procitao = 0;
		while(indeks_procitao < size){

			//buffer je velik kolko i blok
			//u buffer kopiraj blok sa "indeks_bloka"
			DISK_READ(buf, 1, fd->tfd->block[block_start + indeks_bloka]);
			indeks_bloka += 1;

			size_t read = ft->block_size - block_offset;

			//preko # presao
			if(read > (size - indeks_procitao)){
				read = size - indeks_procitao;
			}

			memcpy(buffer+indeks_procitao, buf+block_offset, read);
			//pomakni taman do kraja bloka, na offset 0
			indeks_procitao += read;
			block_offset = 0;
		}
